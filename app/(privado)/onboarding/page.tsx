...
export default function OnboardingPage() {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      // ============================================
      // PASSO 5: Criar categorias padrão e conta padrão
      // Isso garante que o usuário tenha categorias básicas para começar
      // ============================================
      try {
        // Criar categorias padrão
        const { error: categoriasError } = await supabase.rpc("criar_categorias_padrao", {
          usuario_uuid: user.id,
        })

        if (categoriasError) {
          console.warn("Erro ao criar categorias padrão (não crítico):", categoriasError)
          // Não bloqueia o onboarding se falhar
        } else {
          console.log("✅ Categorias padrão criadas com sucesso")
        }

        // Criar conta padrão "Carteira"
        const { error: contaError } = await supabase.rpc("criar_conta_padrao", {
          usuario_uuid: user.id,
        })

        if (contaError) {
          console.warn("Erro ao criar conta padrão (não crítico):", contaError)
          // Não bloqueia o onboarding se falhar
        } else {
          console.log("✅ Conta padrão criada com sucesso")
        }
      } catch (err) {
        console.warn("Erro ao criar dados padrão (não crítico):", err)
        // Não bloqueia o onboarding se falhar
      }
